...
%   catch
%       display(['Error at layer ' num2str(l)]); 
      obj.layers(l).block.forwardAdvanced(obj.layers(l)) ;
%   end
...
% inputs = gather(obj.vars(1).value);    % OrgImage
% init   = gather(obj.vars(4).value);    % Init
% rec    = gather(obj.vars(13).value);    % Rec
% 
% figure(1); subplot(1, 3, 1); imshow(inputs(:, :, 1, 1), []);
% subplot(1, 3, 2); imshow(init(:, :, 1, 1), []);
% subplot(1, 3, 3); imshow(rec(:, :, 1, 1), []);
% -------------------------------------------------------------------------
% Backward pass
% -------------------------------------------------------------------------

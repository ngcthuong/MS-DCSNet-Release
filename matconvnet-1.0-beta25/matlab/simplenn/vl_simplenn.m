    ...
    switch l.type
        case 'conv'
            ...
            res(i+1).x = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
                ...
                display(['Forward pass, Error at layer ' num2str(i)]); 
                pause; 
            end
        case 'convt'
        ...
        case 'custom'
            ...
            % Custorm reshape layer
        case 'reshapeconcat'
            res(i+1).x = vl_nnreshapeconcat(res(i).x, []);
            
        case 'reshape_meas'
            res(i+1).x = vl_nnreshape_meas(res(i).x, []); 
        case 'reshape_hor' 
            res(i+1).x = vl_nnreshapehor(res(i).x, []);
        case 'reshape_ver'
            res(i+1).x = vl_nnreshapever(res(i).x, []);
		case 'bcs_init_rec'
			res(i+1).x = bcs_initialRec_wzhshi( res(i).x, l.dims);  
        case 'rsrm_sampl'
			res(i+1).x = vl_nnrsrmsamp(res(i).x, l, 1); 
        otherwise
        ...
        switch l.type
            ...
            case 'conv'
                try
                    ...
                    % Checking file type; 
                    tmp_Data = res(i).x;
                    tmp_derrOutput = res(i+1).dzdx;
                    tmp_derrOutput_i = res(i).dzdx;
                    
                    display(['Backward Pass, Layer : ' num2str(i), ', ', l.type ', weight size: ' ...
                        num2str(size(l.weights{1})) ', size x: ' num2str(size(res(i).x)) ]);
                    pause; 
                end
            case 'convt'
                [res(i).dzdx, dzdw{1}, dzdw{2}] = ...
            ...
            case 'reshapeconcat'
                res(i).dzdx = vl_nnreshapeconcat(res(i).x, res(i+1).dzdx);
                
            case 'reshape_meas'
                res(i).dzdx = vl_nnreshape_meas(res(i).x, res(i+1).dzdx); 
            
            case 'reshape_hor'
                res(i).dzdx = vl_nnreshapehor(res(i).x, res(i+1).dzdx); 
            
            case 'reshape_ver'
                res(i).dzdx = vl_nnreshapever(res(i).x, res(i+1).dzdx); 
